...
namespace CAN
{
    enum can_ids
    {
        ...
        MOTOR_BOARD_LINEAR_PI_SET,  // MotorBoardPiSet (linear p + i)
        MOTOR_BOARD_ANGULAR_PI_SET, // MotorBoardPiSet (angular p + i)
        MOTOR_BOARD_DERIVATIVE_SET, // MotorBoardDerivativeSet (linear d + angular d)
        // warning: not much room left until message 60
        ...
        C620_CURRENT_COMMAND = 0x200,        // C620Input (ESC id 1 to 4)
        C620_CURRENT_COMMAND_EXTEND = 0x1FF, // C620Input (ESC id 5 to 8)
...
#pragma pack(push, 1) // Ensure tight packing of the struct
    ...
    {
        // array of inputs
        uint8_t digital_inputs; // 1 byte
        uint8_t _unused;        // 1 bytes
        uint16_t _unused2;      // 2 bytes
        uint32_t _unused3;      // 4 bytes
    };

    struct DigitalOutputs
    {
        // array of ouputs
        uint16_t enable_outputs; // 2 bytes
        uint8_t enable_power;    // 1 byte
        uint8_t _unused;         // 1 bytes
        uint32_t _unused2;       // 4 bytes
    };
    ...
    struct MotorBoardPiSet
    {
        float p; // 4 bytes
        float i; // 4 bytes
    }; // 8 bytes — reused for linear and angular

    struct MotorBoardDerivativeSet
    {
        float linear_d;  // 4 bytes
        float angular_d; // 4 bytes
    }; // 8 bytes
